...
export const adaptOpenGraphImages = async (openGraph: OpenGraph = {}, astroSite?: URL): Promise<OpenGraph> => {
  ...
  const resolveBaseUrl = () => {
    ...
    if (typeof envSite === 'string' && envSite.length > 0) {
      try {
        return new URL(envSite);
      } catch {
        return undefined;
      }
    }
    ...
    images.map(async (image) => {
      if (image?.url) {
        ...
        if (typeof _image === 'object') {
          const resolvedSrc = 'src' in _image && typeof _image.src === 'string' ? _image.src : '';
          const absoluteSrc = baseUrl && resolvedSrc ? String(new URL(resolvedSrc, baseUrl)) : resolvedSrc;
          return {
            url: absoluteSrc,
